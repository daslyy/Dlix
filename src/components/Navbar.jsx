  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-colors duration-300 ${
        scrolled ? 'bg-void/95 backdrop-blur-sm shadow-lg shadow-black/40' : 'bg-gradient-to-b from-black/70 to-transparent'
      }`}
    >
      <div className="flex items-center justify-between px-4 sm:px-8 py-3">
        <div className="flex items-center gap-8">
          ...
          <div className="hidden md:flex gap-6 text-sm font-medium text-fog">
            <a href="/" className="hover:text-paper transition-colors">Browse</a>
            <a href="/" className="hover:text-paper transition-colors">My List</a>
          </div>
        </div>
          ...
          <div className="flex items-center">
            <input
              ...
              className={`bg-charcoal/90 text-sm text-paper placeholder:text-fog border border-charcoal-light rounded-full outline-none transition-all duration-300 focus:border-ember ${
                searchOpen ? 'w-40 sm:w-64 px-4 py-1.5 opacity-100' : 'w-0 px-0 py-1.5 opacity-0 border-transparent'
              }`}
            />
            <button
          ...
          <div className="relative">
            ...
            {menuOpen && (
              <div className="absolute right-0 mt-2 w-44 bg-charcoal border border-charcoal-light rounded-md shadow-xl py-2 text-sm">
                <div className="px-4 py-2 text-fog truncate border-b border-charcoal-light">
                  {user?.name}
                </div>
                <button
